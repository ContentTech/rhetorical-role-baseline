...
span_id_dir = "../data/label2id/span_ent2id.json"
span_dict = json.loads(open(span_id_dir).read())

crf_id_dir = "../data/label2id/crf_ent2id.json"
...
judge_train_examples = convert_judgement_data_to_json(max_seq_len, judgement_train_dir, crf_dict, span_dict, "")
judge_dev_examples = convert_judgement_data_to_json(max_seq_len, judgement_dev_dir, crf_dict, span_dict, "")
...
preamble_train_examples = convert_preamble_data_to_json(max_seq_len, preamble_train_dir, crf_dict, span_dict, "")
preamble_dev_examples = convert_preamble_data_to_json(max_seq_len, preamble_dev_dir, crf_dict, span_dict, "")

train_examples = judge_train_examples + preamble_train_examples
dev_examples = judge_dev_examples + preamble_dev_examples
...
def train():
    ...
    max_grad_norm = 1.0
    num_tags = len(preamble_label2name)
            ...
            for idy, batch in enumerate(test_datas):
                ...
                for i in range(len(input_ids)):
                    ...
                    if f1 >= 0.85 and idy==0:
                        ...
                        entities = crf_decode(y_true[:size], token_segs, preamble_label2name)
                        print ("true entity:", entities)
                        pre_entities =  crf_decode(tokens_out[i], token_segs, preamble_label2name)
